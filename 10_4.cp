...
struct Reading{
    Reading(int h, double t, char c) : hour{h}, temperature{t}, scale{c} { }
    int hour;
    double temperature;
    char scale;
};
...
Reading create_reading()
{
    ...
    std::uniform_int_distribution<> scale(0, 1); // for determining scale ('f' or 'c').
    h = hr(eng);
    t = temp(eng);
    if (scale(eng) == 0)
         c = 'c';
    else c = 'f';
    return Reading(h, t, c);
}
...
void fill_vector(vector<Reading>& v, const string& name) // fill vector with random readings.
{
    ...
    int h = 0; int t = 0; char c = 'x';
    while (ist_temps >> h >> t >> c)
        v.push_back(Reading(h, t, c));
}
...
vector<Reading> convert_scale(vector<Reading> r)
{
    for (int i = 0; i<r.size(); ++i){
        if (r[i].scale == 'c')
            r[i].temperature = (r[i].temperature*1.8 +32);
    }
    return r;
}
...
try
{
    ...
    vector<Reading> converted = convert_scale(readings);
    
    cout << "The mean temperature in " << name << " is " << get_mean(converted) << endl;
    cout << "The median temperature in " << name << " is " << get_median(converted) << endl;
}
